...
const frameDidAttach = (state, action) => {
  // let frameProps = frameStateUtil.getFramePropsFromTab(appStoreRenderer.state.get('tabs').find((tab) => tab.get('tabId') == action.frameProps.get('tabId')))
  // return state.mergeIn(['frames', frameStateUtil.getFramePropsIndex(frameStateUtil.getFrames(state), action.frameProps)], frameProps)
  return state
}

const setFrameTabId = (state, action) => {
  let frameProps = frameStateUtil.getFramePropsFromTab(appStoreRenderer.state.get('tabs').find((tab) => tab.get('tabId') == action.tabId))
  frameProps = frameProps.set('tabId', action.tabId)
  return state.mergeIn(['frames', frameStateUtil.getFramePropsIndex(frameStateUtil.getFrames(state), action.frameProps)], frameProps)
}
...
const doAction = (action) => {
  ...
  switch (action.actionType) {
    case windowConstants.WINDOW_SET_STATE:
      ...
      return
    case windowConstants.WINDOW_FRAME_DID_ATTACH:
      windowState = frameDidAttach(windowState, action)
      break
    case windowConstants.WINDOW_SET_FRAME_TAB_ID:
      windowState = setFrameTabId(windowState, action)
      break
    ...
    case windowConstants.WINDOW_CLOSE_FRAME: {
      ...
      const frame = action.frameProps || frameStateUtil.getActiveFrame(windowState)
      removeFrame(frame)
      break
    }
